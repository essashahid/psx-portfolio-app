...
export async function computeRatios(supabase: SupabaseClient, ticker: string): Promise<RatioRow[]> {
  ...
  // A loss makes P/E undefined, not negative. Dividing a positive price by
  // negative earnings yields a number that sorts as "cheap" and reads as a
  // valuation — TRG showed −6.7x against a real loss. Withheld with a reason
  // instead, here in the engine so the header, screener, Overview and Copilot
  // all refuse it together rather than each formatting it their own way.
  const peLossMaking = valEpsAdj !== null && valEpsAdj < 0;
  add("P/E", "Price ÷ EPS", { price, eps: valEpsAdj, eps_basis: valPeriod, share_adjust: epsAdjust }, price !== null && valEpsAdj && valEpsAdj > 0 ? price / valEpsAdj : null, need([["price", price], ["EPS", valEpsAdj]]) ?? (peLossMaking ? "Loss-making period — a price-to-earnings multiple has no meaning against negative earnings." : valEpsAdj === 0 ? "EPS is zero." : null), valPeriod);
  ...
  add("P/E (forward)", "Price ÷ annualized run-rate EPS", { price, eps: annualizedEps }, price !== null && annualizedEps && annualizedEps > 0 ? price / annualizedEps : null, need([["price", price], ["annualized EPS", annualizedEps]]) ?? (annualizedEps !== null && annualizedEps < 0 ? "Loss-making run rate — a forward multiple has no meaning against negative earnings." : annualizedEps === 0 ? "EPS is zero." : null), annualizedEpsPeriod ?? incomePeriod);
