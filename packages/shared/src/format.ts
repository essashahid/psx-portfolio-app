...
// The mobile design specifies these precisely: a true minus rather than a
// hyphen, compact magnitudes on the dense screens, PKR always spelled out.
// They are additive on purpose. The formatters above already render the web
// app's figures and changing those would move numbers on a live page.

/** U+2212. A hyphen is not a minus sign and does not align in a column. */
export const MINUS = "−";

/** Replaces leading ASCII hyphens with a true minus. */
export function trueMinus(text: string): string {
  return text.replace(/-/g, MINUS);
}

/**
 * Magnitude at a glance: 769k, 2.4m, 16k.
 *
 * For headline figures where the exact rupee is noise. Anything a reader might
 * compare precisely keeps its full form through formatMoney.
 */
export function formatCompact(value: number | null | undefined, digits = 1): string {
  if (value === null || value === undefined || Number.isNaN(value)) return "—";
  const sign = value < 0 ? MINUS : "";
  const size = Math.abs(value);
  if (size >= 1_000_000_000) return `${sign}${trimZero(size / 1_000_000_000, digits)}b`;
  if (size >= 1_000_000) return `${sign}${trimZero(size / 1_000_000, digits)}m`;
  if (size >= 1_000) return `${sign}${trimZero(size / 1_000, 0)}k`;
  return `${sign}${trimZero(size, 0)}`;
}

/** Compact with an explicit sign, for a change rather than a level. */
export function formatCompactSigned(value: number | null | undefined, digits = 1): string {
  if (value === null || value === undefined || Number.isNaN(value)) return "—";
  const body = formatCompact(value, digits);
...
/**
 * A percentage with a sign and a true minus. One decimal for portfolio level
 * moves, two where a price is being quoted.
 */
...
/** "PKR 1,234.00". Never Rs. and never the rupee glyph. */
export function formatPkr(value: number | null | undefined, digits = 2): string {
  if (value === null || value === undefined || Number.isNaN(value)) return "—";
  const body = Math.abs(value).toLocaleString("en-PK", {
    minimumFractionDigits: digits,
    maximumFractionDigits: digits,
  });
  return `${value < 0 ? MINUS : ""}PKR ${body}`;
}

/** The grouped figure alone, for where a PKR label already sits alongside. */
export function formatFigure(value: number | null | undefined, digits = 0): string {
  if (value === null || value === undefined || Number.isNaN(value)) return "—";
  const body = Math.abs(value).toLocaleString("en-PK", {
    minimumFractionDigits: digits,
    maximumFractionDigits: digits,
