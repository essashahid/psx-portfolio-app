...
//
//   npx tsx scripts/audits/audit-outlook-data.ts            # print report
//   npx tsx scripts/audits/audit-outlook-data.ts --write    # also write data/generated/outlook-data-audit.json
...
async function main() {
  ...
  if (write) {
    const out = resolve(process.cwd(), "data/generated/outlook-data-audit.json");
    writeFileSync(out, JSON.stringify(report, null, 2));
    console.log(`\nWrote ${out}`);
  } else {
    console.log("\nRe-run with --write to save data/generated/outlook-data-audit.json");
  }
}
