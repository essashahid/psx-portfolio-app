...
// backtest.
//
//   npx tsx scripts/backfills/backfill-universe-eod.ts                    # dry run
//   npx tsx scripts/backfills/backfill-universe-eod.ts --write
//   npx tsx scripts/backfills/backfill-universe-eod.ts --write --limit 50 # partial pass
//   npx tsx scripts/backfills/backfill-universe-eod.ts --write --min-rows 1200
//   npx tsx scripts/backfills/backfill-universe-eod.ts --write --force   # repair missed sessions
...
async function main() {
  ...
  // --force refetches everything regardless of how complete it looks. The
  // completeness check below asks whether a symbol's history is deep, which a
  // symbol can pass while still missing recent sessions: exactly what happens
  // when the daily job misses a day. Repairing that needs the check bypassed.
