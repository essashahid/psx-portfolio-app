 ...
 *
 *   npx tsx scripts/maintenance/agent-reconcile.ts --limit 3 --dry
 *   npx tsx scripts/maintenance/agent-reconcile.ts --limit 20
 *   npx tsx scripts/maintenance/agent-reconcile.ts --ticker NATF
 *
 ...
 *
 *   npx tsx scripts/maintenance/agent-reconcile.ts --calibrate
 *   npx tsx scripts/maintenance/agent-reconcile.ts --calibrate --limit 10
 */
...
//
//   AI_DISABLED=false VISION_DISABLED=false npx tsx scripts/maintenance/agent-reconcile.ts ...
//
...
async function main() {
  ...
  const store = JSON.parse(readFileSync("data/reference/sarmaaya-snapshots.json", "utf8")).snapshots as Record<string, Snap>;
  ...
  try {
    filingsCache = JSON.parse(readFileSync("data/reference/filings-inventory.json", "utf8")).entries ?? {};
  } catch {
    filingsCache = {};
  }
...
Explain the gap and give the correct figures. Use the annual report for the FY row and the interim filing for the quarterly/cumulative rows.`;
  ...
  const outFile = CALIBRATE ? "data/generated/agent-calibration-report.json" : "data/generated/agent-reconcile-report.json";
  writeFileSync(
    outFile,
    JSON.stringify(
      {
