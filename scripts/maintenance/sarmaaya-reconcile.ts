 ...
 *
 *   npx tsx scripts/maintenance/sarmaaya-reconcile.ts                # summary
 *   npx tsx scripts/maintenance/sarmaaya-reconcile.ts --diverge      # list real defects
 *   npx tsx scripts/maintenance/sarmaaya-reconcile.ts --candidates   # verification candidates
 *   npx tsx scripts/maintenance/sarmaaya-reconcile.ts --ticker COLG
 */
...
async function main() {
  ...
  const store = JSON.parse(readFileSync("data/reference/sarmaaya-snapshots.json", "utf8")) as { snapshots: Record<string, Snap> };
