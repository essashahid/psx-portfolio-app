// Ad-hoc diagnostic: compares the holdings table against a recomputed portfolio.
//
// KNOWN BROKEN (pre-dates the repository reorganisation): the dynamic imports
// below reference `@/lib/market-data/fetch-psx-eod` and `fetchEventsForUser`,
// neither of which has ever existed in this repository. Repair those two
// references before running it. Run from the repository root:
//   npx tsx scripts/verification/check-holdings-discrepancy.ts
...
async function run() {
  ...
  const { getPortfolio } = await import("@/lib/portfolio/positions");
  ...
  const { fetchEventsForUser } = await import("@/lib/engine/allocation/data");
  const { buildBenchmarkSeries } = await import("@/lib/engine/benchmark-growth");
  const { fetchPsxEod } = await import("@/lib/market-data/fetch-psx-eod");
  const { ensureEodCached, eodCache } = await import("@/lib/market-data/eod-cache");
