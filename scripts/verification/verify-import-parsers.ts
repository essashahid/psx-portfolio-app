...
(async () => {
  await testFile("samples/sample-holdings-akd.csv", "holdings");
  const trades = await testFile("samples/sample-trades-akd.csv", "trades");
  await testFile("samples/sample-dividends-cdc.csv", "dividends");
