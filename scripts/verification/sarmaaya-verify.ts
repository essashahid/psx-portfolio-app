 ...
 * and parse it here into data/reference/sarmaaya-snapshots.json, then diff that reference
 * against our own ratio engine to catch stale/wrong data.
 *
 *   # add or refresh one ticker from pasted snapshot HTML
 *   npx tsx scripts/verification/sarmaaya-verify.ts add CCM /tmp/ccm.html
 *
 *   # verify every stored snapshot against our company_ratios
 *   npx tsx scripts/verification/sarmaaya-verify.ts
 *
...
const STORE = resolve(__dirname, "../data/reference/sarmaaya-snapshots.json");
