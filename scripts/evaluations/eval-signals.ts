...
//
//   npx tsx scripts/evaluations/eval-signals.ts            # print report
//   npx tsx scripts/evaluations/eval-signals.ts --write    # also write data/generated/outlook-signal-evidence.json
...
async function main() {
  ...
  if (write) {
    const out = resolve(process.cwd(), "data/generated/outlook-signal-evidence.json");
    writeFileSync(out, JSON.stringify(report, null, 2));
    console.log(`\nWrote ${out}`);
  } else {
    console.log("\nRe-run with --write to save data/generated/outlook-signal-evidence.json");
  }
}
