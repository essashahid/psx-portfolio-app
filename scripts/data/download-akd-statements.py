"""Download AKD Securities account-statement PDFs from a Gmail mailbox.

Reads GMAIL_PASSWORD (a Gmail app password) from .env.local at the repository
root and writes the attachments to ~/Downloads/akdsl-pdfs. Read-only against
the mailbox. The mailbox address is hardcoded in EMAIL below.
...
# Load from .env.local at the repository root (two levels up from scripts/data/).
