...
function DivergingBar({ label, value, max, unit }: { label: string; value: number | null; max: number; unit: string }) {
  ...
  return (
    <div className="flex items-center gap-2 text-[12px]">
      <span className="w-28 shrink-0 truncate text-text-muted" title={label}>{label}</span>
      ...
      <span className={cn("w-14 shrink-0 text-right font-semibold tabular-nums", positive ? "text-up" : v < 0 ? "text-down" : "text-text-muted")}>
        {fmtFlow(value)}
      </span>
      <span className="hidden w-12 shrink-0 text-[10px] text-text-muted sm:inline">{unit}</span>
    </div>
  );
}
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          <div>
            ...
            <p className={cn("mt-0.5 text-2xl font-semibold tabular-nums tracking-tight", (day.fipiNet ?? 0) > 0 ? "text-up" : (day.fipiNet ?? 0) < 0 ? "text-down" : "")}>
              {fmtFlow(day.fipiNet)} <span className="text-sm font-normal text-text-muted">{unit}</span>
            </p>
          </div>
        </div>
      ...
      {buckets.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-[11px] font-medium text-text-muted">Where foreigners moved, by regime bucket</p>
          {buckets.map((b) => (
      ...
      {!compact && sectors.length > 0 && (
        <div className="space-y-1.5 border-t border-rule pt-3">
          <p className="text-[11px] font-medium text-text-muted">By sector</p>
      ...
      {!compact && participants.length > 0 && (
        <div className="space-y-1.5 border-t border-rule pt-3">
          <p className="text-[11px] font-medium text-text-muted">Local investors (LIPI), net</p>
