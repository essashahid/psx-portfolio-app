...
import { AllocationPie } from "@/components/shared/charts";
  ...
  if (!forecast) {
    return (
      <div className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-dashed border-rule bg-surface-sunken/30 px-4 py-4">
  ...
  return (
    <div className="space-y-4">
      ...
      {forecast.narrative?.summary && (
        <p className="rise text-sm leading-relaxed text-text-strong/85">{forecast.narrative.summary}</p>
      )}
...
function ConfidenceStrip({ forecast }: { forecast: AllocationForecast }) {
  ...
  return (
    <div className="rise rounded-lg border border-rule bg-surface-raised p-3 shadow-[var(--shadow-card)]">
      <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-text-muted">(gated by the weakest component)</span>
        </div>
          ...
          {c.components.map((comp) => (
            <div key={comp.id} className="flex items-center gap-1.5" title={comp.detail}>
              ...
              <span className="text-xs text-text-muted">
                {comp.label}: <span className="text-text-strong/80">{comp.level}</span>
              </span>
            </div>
          ))}
        </div>
    ...
    <Card className="rise">
      <CardHeader>
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <span className="text-xs text-text-muted">Lead scenario: {r.label}</span>
        </div>
      </CardHeader>
      <CardContent className="grid gap-5 md:grid-cols-[260px_1fr]">
        ...
        <div className="space-y-3">
          <div className="rounded-md bg-surface-sunken/40 px-3 py-2">
            <p className="text-sm">
          ...
          {forecast.narrative?.recommendationNote && (
            <p className="text-sm leading-relaxed text-text-strong/80">{forecast.narrative.recommendationNote}</p>
          )}
        </div>
      </CardContent>
    </Card>
...
function OutcomeGrid({ outcome }: { outcome: MixOutcome }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
      {items.map((it) => (
        <div key={it.label} className="rounded-md border border-rule bg-surface-raised px-2.5 py-2">
          <p className="text-[10px] uppercase tracking-wide text-text-muted">{it.label}</p>
      ...
      <CardContent className="space-y-3 p-4">
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-text-muted">P(regime)</p>
          </div>
          <p className="text-xl font-semibold tabular-nums">{pct(s.probability)}</p>
        </div>

        <p className="text-sm leading-relaxed text-text-strong/80">{note ?? s.thesis}</p>

        <WeightBar allocation={s.mix.allocation} />

        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-text-muted">
          <span>Exp <span className="font-medium text-text-strong/80 tabular-nums">{pct(s.mix.expReturn)}</span> ({pct(s.mix.expReturnLow)}–{pct(s.mix.expReturnHigh)})</span>
          <span>Vol <span className="font-medium text-text-strong/80 tabular-nums">{pct(s.mix.volatility)}</span></span>
          <span>Drawdown <span className="font-medium text-text-strong/80 tabular-nums">{pct(s.mix.estDrawdown)}</span></span>
          <span>P(loss) <span className="font-medium text-text-strong/80 tabular-nums">{pct(s.mix.probLoss)}</span></span>
        </div>
      ...
      <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-0.5">
        {ASSET_CLASSES.filter((a) => allocation[a] >= 0.005).map((a) => (
          <span key={a} className="flex items-center gap-1 text-[11px] text-text-muted">
            ...
            {ASSET_LABEL[a]} <span className="tabular-nums text-text-strong/75">{pct(allocation[a], 0)}</span>
          </span>
        ))}
      </div>
    </div>
...
function DeploymentPanel({ forecast }: { forecast: AllocationForecast }) {
  ...
  return (
    <Card className="rise">
      ...
      <CardContent>
        {buys.length === 0 ? (
          <p className="text-sm text-text-muted">Your current mix is already close to the target. No major buys are needed.</p>
        ) : (
...
function Disclosure({ title, children, defaultOpen = false }: { title: string; children: React.ReactNode; defaultOpen?: boolean }) {
  return (
    <details open={defaultOpen} className="rise rounded-lg border border-rule bg-surface-raised shadow-[var(--shadow-card)]">
        ...
        {forecast.signals.map((s) => (
          <li key={s.id}>
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="text-[11px] uppercase tracking-wide text-text-muted">{s.reliability}</span>
            </div>
            <div className="mt-1 flex items-center gap-2">
              <div className="relative h-1.5 flex-1 rounded-full bg-surface-sunken">
                ...
                />
                <div className="absolute left-1/2 top-[-2px] h-2.5 w-px bg-rule" />
              </div>
            </div>
      ...
      <div className="space-y-3 text-sm">
        <div className="space-y-1">
          {[bt.core, bt.fullUniverse, bt.signalOverlap].map((L) => (
            <p key={L.label} className="text-xs text-text-muted">
              <span className="text-text-strong/80">{L.label}:</span> {L.firstMonth} to {L.lastMonth} ({L.observations} obs)
            </p>
          ))}
          <p className="text-xs text-text-muted">{bt.note}</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
  ...
  return (
    <Disclosure title="Structured geopolitical events">
      ...
      {forecast.narrative?.eventsNote && (
        <p className="mt-2 text-xs text-text-muted">{forecast.narrative.eventsNote}</p>
      )}
    </Disclosure>
  );
}
