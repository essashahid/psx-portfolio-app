...
export function CompanyReportViewer({
  ...
  const [openSections, setOpenSections] = useState<Set<string>>(new Set(["executive", "financials", "peers", "price"]));
  ...
  return (
    <div className="rounded-md border border-border">
      ...
      <nav className="flex flex-wrap gap-1 border-b border-border px-3 py-2">
        {["price", "executive", "financials", "valuation", "peers", "portfolio", "scenarios", "news"].map((id) => (
          <button
            key={id}
            type="button"
            onClick={() => toggle(id)}
      ...
      <div className="max-h-[32rem] overflow-y-auto p-3 space-y-1">
        ...
        >
          ...
          <InsightBlock title="Business Overview" items={payload.narrative.businessOverview} />
        ...
        >
          ...
          <InsightBlock title="Financial Quality" items={payload.narrative.financialQuality} />
        </CollapsibleSection>
        ...
        <CollapsibleSection id="news" title="News and Filings" open={openSections.has("news")} onToggle={toggle} refresh={<SectionRefreshButton reportId={reportId} sectionId="news" onUpdated={setPayload} />}>
          <InsightBlock title="Recent developments" items={payload.narrative.recentDevelopments} />
          <ul className="mt-2 space-y-1 text-xs text-muted-foreground">
            {((payload.evidence.independentNews as { title: string; publishedAt?: string | null }[]) ?? []).slice(0, 8).map((n) => (
              <li key={n.title}>{n.publishedAt?.slice(0, 10) ?? "—"} · {n.title}</li>
            ))}
          </ul>
        </CollapsibleSection>
