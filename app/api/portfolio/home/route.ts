...
export async function GET() {
  ...
  try {
    ...
    const sectors: HomeSectorSlice[] = portfolio.sectorWeights
      .map((s) => ({
        sector: s.sector,
        value: s.value,
        weightPct: s.weight,
    ...
    const body: HomeResponse = {
      ...
      atCost: portfolio.pricedHoldings === 0 && portfolio.holdingsCount > 0,
      largest: portfolio.largestHolding
        ? {
            ticker: portfolio.largestHolding.ticker,
            weightPct: portfolio.largestHolding.weight ?? 0,
          }
        : null,
      sectors,
      contributors,
